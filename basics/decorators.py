...
def subscribe(receiver):
    """
    Parameterized decorator to register a function or class for some purpose.

    This allows a function or class to be defined and, at the same time, set up
    as an event handler, or added to a list of functions of some kind, or any
    other logic the receiver implements.

    How the function or class is registered depends on what operations the
    receiver supports. The ways, from most to least preferred, are calling the
    receiver, appending to the receiver as if it is a MutableSequence, and
    adding to the receiver as if it is a MutableSet. EAFP is used, not LBYL. If
    none of these operations is supported, TypeError is raised.

    NOTE: This is as much about EAFP design as about decorators, and the tests
    are deliberately non-exhaustive. Think about what behavior is reasonable
    for errors that don't represent the complete absence of a feature. For
    example, if the receiver is a non-callable mutable set that only accepts
    numeric elements, this should fail with the TypeError the receiver raises
    about that (and nothing with an append method should ever be attempted).
    You should also take reasonable steps to reduce the likelihood of handling
    any exceptions that are raised due to bugs in code you don't control. Some,
    but not all, of the design decisions you'll make here may be subjective. It
    is permitted but not required to fail fast, i.e., in the call to subscribe.

    >>> @subscribe(lambda func: print(func.__name__))
    ... def fn(): pass
    fn

    >>> all_click_handlers = []; some_click_handlers = set()
    >>> @subscribe(all_click_handlers)
    ... def f(x, y, button, **options): pass
    >>> @subscribe(all_click_handlers)
    ... @subscribe(some_click_handlers)
    ... def g(x, y, button, **options): pass
    >>> @subscribe(some_click_handlers)
    ... @subscribe(lambda func: print(func.__name__))
    ... @subscribe(all_click_handlers)
    ... @subscribe(all_click_handlers)  # Subscribed twice.
    ... def h(x, y, button, **options): pass
    h
    >>> all_click_handlers == [f, g, h, h]
    True
    >>> some_click_handlers == {g, h}
    True
    ...
    def decorator(func):
        with contextlib.suppress(TypeError):
            # It would probably be better to access the __call__ attribute.
            receiver(func)
            return func
        ...
        try:
            ...
            raise TypeError("call, append, and add all unsupported") from error
